...
exports.signUp = async (req, res) => {
    try {
        ...
        const userExits = await userModel.findOne({ email: email.toLowerCase() });
        if (userExits) {
...
exports.resendVerification = async (req, res) => {
    try {
        ...
        // Find the user and check if the user still exists
...
exports.getAll = async (req, res) => {
    try {
        ...
        // Send a success response
        res.status(200).json({
            message: `All user's in the database and the total is: ${users.length}`,
            data: users
        })
    } catch (error) {
        res.status(500).json({
